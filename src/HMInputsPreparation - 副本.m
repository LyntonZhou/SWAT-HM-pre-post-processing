...
PointSourceNo=[47,48,49]; % if icc(4)=1, then PointSourceNo is the No of point source files
promdir = pwd;
xlsdir = [promdir '\HeavyMetalModuleDataBase_XiangRiver.xlsx']; % database excel file directory
projdir = 'D:\XiangRiverProject\XiangRiver\XRB210418V2\Scenarios\Default\TxtInOut - ¸±±¾'; % folder of SWAT-HM project
...
for ii=1:numel(icc)
    ...
    if (ii==4 && icc(ii)==1)
        for jj=1:numel(PointSourceNo)
            psxlsname=['Hml_Point_' num2str(PointSourceNo(jj)) 'p'];
            [num, ~, ~] = xlsread(xlsdir,psxlsname);
            HmlPoint = num;
            ...
            for kk=1:numel(HmlPoint(:,1))
                fprintf(psfileid,formatSpec,num2str(HmlPoint(kk,1)),num2str(HmlPoint(kk,2)),HmlPoint(kk,3:4),...
                    zeros(1,13),HmlPoint(kk,5),zeros(1,2));
            end
            fclose(psfileid);
        end
        disp([num2str(numel(PointSourceNo)) ' point source files are modified'])
        clearvars kk num psxlsname psfilename psfileid formatSpec Notes
    end
end
toc
