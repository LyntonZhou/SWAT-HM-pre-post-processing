...
def hml_hru(table_info):
    ...
    for i in range(len(hru_list)):
        ...
        hml_lines.append(f"{round(hml_data['Total hm in Fertilizers'][i],3).astype(str).rjust(10)}    |HML_P7: Total metal input from agricultural use [g/ha/yr]\n")
        hml_lines.append(f"{round(hml_data['Fraction of labile hm in Fertilizers'][i],3).astype(str).rjust(10)}    |HML_P8: Fraction of labile metal in fertilizers or animal manure [-]\n")
        hml_lines.append(f"{round(hml_data['Total hm in atmospheric deposition'][i],3).astype(str).rjust(10)}    |HML_P9: Total metal input from atmospheric deposition [g/ha/yr]\n")
        hml_lines.append(f"{round(hml_data['Fraction of labile hm in atmospheric deposition'][i],3).astype(str).rjust(10)}    |HML_P10: Fraction of labile metal in atmospheric deposition [-]\n")
        hml_lines.append(f"{round(hml_data['Dissolved hm in groundwater'][i],3).astype(str).rjust(10)}    |HML_P11: Dissolved metal in groundwater [ug/L]\n")
