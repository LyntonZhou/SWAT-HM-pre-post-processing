...
txtiodir = 'D:\XiangRiverProject\XiangRiver\XRB210528V2_1.Sufi2.SwatCup';
...
outdir = 'D:\XiangRiverProject\XiangRiver\XRB210528V2\Outputs\figs\figV2_1\fig12';
...
SimSed=permute(reshape(Sim(:,[2,3,9]),SubNo,Simlength,3),[2 3 1]);
...
for ii=1:numel(SedStations(:,1))
    if sum(~isnan(ObsSed(:,ii+2)))< 1
        ...
        leg(ii) = scatter(ObsSed(Lib(:,1),ii+2)*86.4.*lastdayofmonth(Lic(:,1),3),SimSed(Lia(:,1),3,SubLoc2(ii,1)),30,[colorsed(ii,:)],'filled');
